#!/bin/bash
APP_NAME=hyb-ct-ums.jar
APP_BASEPATH=/data/webapps/hyb-ct-ums
JARPATH=$APP_BASEPATH/source/hyb-ct-ums.jar
...
start(){
  ...
  if [ $? -eq "0" ]; then
    ...
    mvn clean install  -Dmaven.test.skip=true -P$AppActive && mv $APP_BASEPATH/source/hyb-ct-service/hyb-ct-ums/target/hyb-ct-ums-1.0.jar  $JARPATH
    echo "make finish ..."
    chmod  777 $JARPATH
